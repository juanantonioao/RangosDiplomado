...
public class Rangos {
	...
	// Variable para almacenar el texto del último Jbutton que se haya pulsado, para
	// que no se pierda al pulsar otro botón
	...
	private void initialize() {
		...
		// Tamaño que tendrá la ventana cuando quitemos el maximizado (setPreferredSize
		// no lo aplica):
		...
		// Componentes (unos pocos los he declarado fuera de initialize() para poder
		// acceder a ellos desde métodos y funciones de la clase Rangos
		...
		// Listener común para todos los botones de los tramos y otro para los
		// radiobuttons (se asignan en un for más abajo)
		...
		// Tiene su propio layout para que los botones se alineen así: 
		// 3b - 3bo <ESPACIO> 5 botones <ESPACIO> 4bo - 4b 
		// independientemente de la cuadrícula principal
		...
		for (int cont = 0; cont < botonesTramos.size(); cont++) {
			cTramos.gridx = cont;
			// El primer y el último botón tienen insets propios, más anchos en la parte que
			// toca al margen de la ventana
			if (cont == 0)
				cTramos.insets = iIzq;
			else if (cont == (botonesTramos.size() - 1))
				cTramos.insets = iDer;
			else
				cTramos.insets = iCen;
			frmRangosDiplomado.getContentPane().add(botonesTramos.get(cont), cTramos);
			// Sin el SetPreferredSize y el SetMinimumSize, los anchos de los botones no se
			// quedan fijos sino que van adaptándose al botón seleccionado
		...
		// Creación y colocación del panel de radiobuttons de 3bet en el layout
		// principal
		...
		for (int cont = 0; cont < buttons3b.size(); cont++) {
			...
			// Aprovecho este bucle de la parte gráfica para añadir los radiobuttons al
			// ButtonGroup, deshabilitarlos por defecto y añadir el listener
		...
		// Creación y colocación del panel de radiobuttons de 4bet en el layout
		// principal
	...
	private void dibujarRango(String ruta) {
		try {
			...
			// Código para evitar que la imagen sobresalga por arriba/abajo o por los lados,
			// calculando previamente la proporción
			...
			if (propImagen <= propEspacio)
				...
				imagen = iconoTemporal.getImage().getScaledInstance(espacio.getWidth(), -1, Image.SCALE_SMOOTH);
			ImageIcon icono = new ImageIcon(imagen);
			espacio.setIcon(icono);
		} catch (NullPointerException err) {
			// Si la ruta no existe, o no hay ninguna imagen en dicha ruta, se deja sin
			// imagen
	...
	private String obtenerRuta() {
		...
		// grupoBotones sirve para poder detectar el RadioButton seleccionado usando un
		// único bucle que sirva tanto para el 3b como para el 4b
	...
	private void cursivaBotonTramo() {
		String numeroBoton;
		try {
			// La función substring genera una excepción si la cadena está vacía, de ahí el
			// try. Uso substring para poder seleccionar el botón según el número por el que
			// empieza su nombre (01. 02. etc.)
			if (btn3b4b.equals(nombrebtn3b) || btn3b4b.equals(nombrebtn3bo))
				numeroBoton = btnRango3b.substring(0, 2);
			else if (btn3b4b.equals(nombrebtn4b) || btn3b4b.equals(nombrebtn4bo))
				numeroBoton = btnRango4b.substring(0, 2);
			else
				return; // Este return es útil cuando pulsemos uno de los botones centrales del panel
						// superior, que no ponen nada en cursiva
			for (int cont = 0; cont < botonesTramos.size(); cont++) {
